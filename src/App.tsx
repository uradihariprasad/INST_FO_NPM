...
import { runFullScan, generateMarketBreadth, generateSectorData, isUsingLiveData } from './engine/scanner';
...
import { Play, Pause, RefreshCw, Zap, Wifi, WifiOff } from 'lucide-react';
...
export default function App() {
  ...
  const performScan = useCallback(async () => {
    ...
    try {
      ...
      setDataMode(isUsingLiveData() ? 'live' : 'simulated');
      setConnected(isUsingLiveData());
      setConnectionError('');
    } catch (error) {
  ...
  }, [apiToken, setScannerResults, setMarketBreadth, setSectorData, setLastScanTime, setIsScanning, setConnected, setConnectionError]);
      ...
      <div className="min-h-screen bg-gray-950 flex items-center justify-center">
        <div className="text-center animate-fade-in">
          ...
          <p className="text-xs text-gray-600 mt-3">
            {apiToken ? 'Connecting to Upstox API...' : 'Loading simulated data...'}
          </p>
        </div>
      </div>
    );
